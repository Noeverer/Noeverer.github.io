...
class MarkdownConverter:
    ...
    def parse_html_file(self, filepath):
        ...
        try:
            ...
            # 如果提取到了内容
            if title and article_content.strip():
                article_info = {
                    'title': title,
                ...
                print(f"✓ [{category}] {title} (Content length: {len(article_content)})")
                return article_info
            elif title:
                # 即使内容为空也记录，但给出警告
                print(f"⚠ [{category}] {title} (Empty content)")
                article_info = {
                    'title': title,
                    'date': date or '2019-08-01',
                    'tags': tags,
    ...
    def extract_content_from_html(self, soup, filepath):
        ...
        if not content_element:
            ...
            if body:
                # 移除侧边栏等非内容区域
                for unwanted in body.select('.left-col, .overlay, .mid-col, .right-col, header, footer, nav, .header-menu, .article-meta, .page-reward, .article-info'):
                    unwanted.decompose()
    ...
    def parse_element(self, element):
        ...
        for child in element.children:
            if child.name is None:
                ...
                # 检查是否是标题类的div
                if child.get('class') and any('title' in cls or 'header' in cls for cls in child.get('class', [])):
                    continue  # 跳过标题类的div，避免重复
                # 递归处理容器元素
